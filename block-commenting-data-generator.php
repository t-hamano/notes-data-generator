<?php
/**
 * Plugin Name: Block Commenting Data Generator
...
function bcdg_activate() {
	...
	for ( $i = 1; $i <= 100; $i++ ) {
		...
		if ( ! is_wp_error( $user_id ) ) {
			++$created_count;
			update_user_meta( $user_id, 'bcdg_generated_user', '1' );
			wp_update_user(
				array(
					'ID'           => $user_id,
	...
	set_transient( 'bcdg_activation_notice', $created_count, 60 );
}
register_activation_hook( __FILE__, 'bcdg_activate' );

/**
 * Delete all test users and test block comments when the plugin is deactivated.
 */
function bcdg_deactivate() {
	$users = get_users(
		array(
			'search'         => 'test_user_*',
...
register_deactivation_hook( __FILE__, 'bcdg_deactivate' );

/**
 * Enqueue assets.
 */
function bcdg_enqueue_assets() {
...
add_action( 'enqueue_block_editor_assets', 'bcdg_enqueue_assets' );

/**
 * Display notice message when plugin is activated.
 */
function bcdg_show_activation_notice() {
	$notice_data = get_transient( 'bcdg_activation_notice' );
	if ( $notice_data ) {
		printf(
			'<div class="notice notice-success is-dismissible"><p><strong>Block Commenting Data Generator:</strong> %d users created.</p></div>',
			$notice_data
		);
		delete_transient( 'bcdg_activation_notice' );
	}
}
add_action( 'admin_notices', 'bcdg_show_activation_notice' );

/**
 * Display confirmation message when plugin is deactivated.
 */
function bcdg_show_deactivation_notice() {
	?>
	...
	document.addEventListener( 'DOMContentLoaded', function() {
		...
		deactivateLinks.forEach( function(link ) {
			if ( link.getAttribute('id') === 'deactivate-block-commenting-data-generator' ) {
...
add_action( 'admin_footer-plugins.php', 'bcdg_show_deactivation_notice' );

/**
 * Override the avatar URL for test users.
 */
function bcdg_get_avatar_url( $avatar_url, $id_or_email, $args ) {
	if ( ! is_numeric( $id_or_email ) ) {
		return $avatar_url;
	}
	return 'https://secure.gravatar.com/avatar/' . $id_or_email . '?s=' . $args['size'] . '&d=wavatar';
}
add_filter( 'get_avatar_url', 'bcdg_get_avatar_url', 10, 3 );

/**
 * Override the avatar URL for test users in the REST API.
 */
function bcdg_rest_prepare_comment( $response, $comment, $request ) {
...
add_filter( 'rest_prepare_comment', 'bcdg_rest_prepare_comment', 10, 3 );
